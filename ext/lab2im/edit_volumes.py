...
def crop_volume_around_region(volume,
    ...
    if n_dims is None:
        ...
        if len(new_vol.shape) == n_dims + 1:
            n_channels = new_vol.shape[-1]
        elif len(new_vol.shape) == n_dims:
            n_channels = 1
        else:
            raise ValueError('input volume has 2 more dimensions than provided n_dims.')
    ...
    if aff is not None:
        ...
        return new_vol, cropping, pad_margins, aff
    else:
        return new_vol, cropping, pad_margins
            ...
            if incorrect_label in volume_labels:
                ...
                # several possibilities
                    ...
                    while correct_label_not_found:
                        tmp_labels, crop, _ = crop_volume_around_region(labels,
        ...
        for incorrect_label in list_incorrect_labels:
            if incorrect_label in volume_labels:
                ...
                for i in range(1, n_components + 1):
                    ...
                    # crop each region
                    _, crop, _ = crop_volume_around_region(components, masking_labels=i, margin=1)
...
def erode_label_map(labels, labels_to_erode, erosion_factors=1., gpu=False, model=None, return_model=False):
    ...
    for label_to_erode, erosion_factor in zip(labels_to_erode, erosion_factors):
        ...
        cropped_lab_mask, cropping, _ = crop_volume_around_region(mask, margin=3)
...
def compute_distance_map(labels, masking_labels=None, crop_margin=None):
    ...
    if crop_margin is not None:
        tmp_labels, crop_idx, _ = crop_volume_around_region(labels, margin=crop_margin)
    else:
        tmp_labels = labels
        crop_idx = None
...
def labels_to_regions(labels, labels_to_regions_indices):
    ...
    n_dims, _ = utils.get_dims(labels.shape, max_channels=2)

    # compute one-hot
    lab_one_hot = (np.arange(n_regions + 1) == labels[..., None]).astype(int)
    channel_masking = np.sum(lab_one_hot, tuple(range(n_dims))) > 0
    list_main_label_per_region = [np.where(labels_to_regions_indices[:, i] > 0)[0][0] for i in range(n_regions)]

    # compute multi-hot by summing and stack into channels
    new_labels = list()
    for i in range(n_regions):
        if channel_masking[list_main_label_per_region[i]]:
            tmp_lab = np.sum(lab_one_hot * utils.add_axis(labels_to_regions_indices[..., i], [0] * 3), axis=-1)
            new_labels.append(tmp_lab)
        else:
            new_labels.append(np.zeros_like(lab_one_hot[..., 0]))
    new_labels = np.stack(new_labels, axis=-1)
...
class spatialMeans:
    ...
    def __init__(self, n_dims, eps=1e-9):
        self.n_dims = n_dims
        self.eps = eps
        self.coords = None
        self.size_in = None
    ...
    def build_coord(self):
        coords = np.meshgrid(*[np.arange(0, ss) for ss in self.size_in], indexing='ij')
        coords = [np.reshape(cc, [-1]) for cc in coords]
        coords = np.stack(coords)
        coords = utils.add_axis(coords, -1)  # size (3, H*W*D, 1)
        return coords

    def get_centroids(self, x, labels_list):

        # convert x to soft probabilities
        n_labels = len(labels_list)
        x_n_dims, x_n_channels = utils.get_dims(x.shape, max_channels=n_labels + 1)
        assert x_n_dims == self.n_dims, 'input should have %s dims, had %s' % (self.n_dims, x_n_dims)
        if x_n_channels == 1:  # hard segmentation
            lut = utils.get_mapping_lut(labels_list, np.arange(n_labels))
            x = lut[x]
            x = (np.arange(n_labels) == x[..., None]).astype(int)
        elif x_n_channels != n_labels:
            raise ValueError('input with %s channels does not match expected %s labels' % (x_n_channels, n_labels))

        # rebuild coord meshgrid if shape is different from before
        if (self.size_in is None) or x.shape[:self.n_dims] != self.size_in:
            self.size_in = x.shape[:self.n_dims]
            self.coords = self.build_coord()

        # compute spatial means, size ()
        x = np.reshape(x, [-1, n_labels])
        x = utils.add_axis(x, 0)  # size (1, H*W*D, n_labels)
        top = np.sum(x * self.coords, axis=1)
        bottom = np.sum(x, axis=1) + self.eps
        return top / bottom  # size (3, n_labels)
    ...
    for idx, (path_image, path_mask) in enumerate(zip(path_images, path_masks)):
        ...
        if (not os.path.isfile(path_result)) | recompute:
            ...
            if path_mask is not None:
                ...
                mask = None
            volume, cropping, _, aff = crop_volume_around_region(volume, mask, threshold, masking_labels, crop_margin, aff)
            utils.save_volume(volume, aff, h, path_result)
...
def niftyreg_images_in_dir(image_dir,
    ...
    for idx, (path_image, path_ref, path_input_trans) in enumerate(zip(path_images,
        ...
        if (not result_already_computed) | (not result_trans_already_computed) | recompute:
            ...
            if interpolation is not None:
                if nifty_reg_function == 'reg_aladin':
                    cmd = utils.mkcmd(cmd, '-interp', interpolation)
                else:
                    cmd = utils.mkcmd(cmd, '-inter', interpolation)
            if rigid_only & (nifty_reg_function == 'reg_aladin'):
                cmd = utils.mkcmd(cmd, '-rigOnly')
            if addon_string is not None:
                cmd = utils.mkcmd(cmd, addon_string)
...
def smooth_labels_in_dir(labels_dir, result_dir, gpu=False, labels_list=None, connectivity=1, recompute=True):
    ...
    if labels_list is not None:
        labels_list, _ = utils.get_list_labels(label_list=labels_list, FS_sort=False)
...
def crop_dataset_to_minimum_size(labels_dir, result_dir, image_dir=None, image_result_dir=None, margin=5):
    ...
    for idx, (path_label, path_image) in enumerate(zip(path_labels, path_images)):
        ...
        label, cropping, _, aff = crop_volume_around_region(label, aff=aff)
...
def crop_dataset_around_region_of_same_size(labels_dir,
    ...
    if recompute_labels:
        for path_label in path_labels:
            ...
            _, cropping, _ = crop_volume_around_region(label)
